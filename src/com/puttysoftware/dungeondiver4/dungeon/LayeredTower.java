...
class LayeredTower implements Cloneable {
	...
	this.savedTowerState = new LowLevelADODataStore(cols, rows, floors, DungeonConstants.LAYER_COUNT);
	...
	copy.savedTowerState = (LowLevelADODataStore) this.savedTowerState.clone();
	...
	try {
		...
		this.setCell(block.getSavedObject(), xLoc, yLoc, zLoc, DungeonConstants.LAYER_OBJECT);
		// Move the block
		block.setSavedObject(there);
		this.setCell(block, xLoc + dirMove[0], yLoc + dirMove[1], zLoc, DungeonConstants.LAYER_OBJECT);
    ...
    public void updateMonsterPosition(final int move, final int xLoc, final int yLoc, final MonsterObject monster) {
	final int[] dirMove = DirectionResolver.unresolveRelativeDirection(move);
    ...
    public void postBattle(final MonsterObject m, final int xLoc, final int yLoc, final boolean player) {
	final AbstractDungeonObject saved = m.getSavedObject();
	final int zLoc = this.getPlayerFloor();
	if (player) {
	    DungeonDiver4.getApplication().getGameManager().setSavedDungeonObject(saved);
	} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
			...
			// Kill the monster
			final MonsterObject m = (MonsterObject) this.getCell(u, v, z, l);
			this.setCell(m.getSavedObject(), u, v, z, l);
			// Reward player for monster death
			DungeonDiver4.getApplication().getBattle().doBattleByProxy();
			// Respawn it
			this.generateOneMonster();
		    }
		} catch (final ArrayIndexOutOfBoundsException aioob) {
    ...
    public void findAllObjectPairsAndSwap(final AbstractDungeonObject o1, final AbstractDungeonObject o2) {
	int y, x, z;
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		for (z = 0; z < this.getFloors(); z++) {
		    final AbstractDungeonObject mo = this.getCell(y, x, z, DungeonConstants.LAYER_OBJECT);
		    if (mo != null) {
			if (mo.getName().equals(o1.getName())) {
			    this.setCell(o2, y, x, z, DungeonConstants.LAYER_OBJECT);
			} else if (mo.getName().equals(o2.getName())) {
			    this.setCell(o1, y, x, z, DungeonConstants.LAYER_OBJECT);
			}
		    }
		}
	    }
	...
	final LowLevelADODataStore tempStorage = new LowLevelADODataStore(y, x, z, DungeonConstants.LAYER_COUNT);
	...
	this.savedTowerState = new LowLevelADODataStore(y, x, z, DungeonConstants.LAYER_COUNT);
    }
	...
	if (xDist <= this.visionRadius && yDist <= this.visionRadius) {
	    if (this.radialScanVision(x2, y2, zLoc, DungeonConstants.LAYER_OBJECT,
    ...
    public void fillFloorRandomly(final Dungeon dungeon, final int z, final int w) {
	// Pre-Pass
	final DungeonObjectList objects = DungeonDiver4.getApplication().getObjects();
	final AbstractDungeonObject pass1FillBottom = PreferencesManager.getEditorDefaultFill();
	...
	for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
		...
		for (x = 0; x < columns; x++) {
			...
			final boolean okay = placeObj.shouldGenerateObject(dungeon, x, y, z, w, e);
			if (okay) {
	...
	for (int layer = 0; layer < DungeonConstants.LAYER_COUNT; layer++) {
		...
		for (x = 0; x < requiredObjects.length; x++) {
			...
			randomRow = row.generate();
			randomColumn = column.generate();
			if (currObj.shouldGenerateObject(dungeon, randomRow, randomColumn, z, w, layer)) {
			    this.setCell(objects.getNewInstanceByName(currObj.getName()), randomColumn, randomRow, z,
				    layer);
			    currObj.editorGenerateHook(y, x, z);
			} else {
			    while (!currObj.shouldGenerateObject(dungeon, randomColumn, randomRow, z, w, layer)) {
				randomRow = row.generate();
				randomColumn = column.generate();
			    }
			    this.setCell(objects.getNewInstanceByName(currObj.getName()), randomColumn, randomRow, z,
    ...
    public void fillFloorRandomlyCustom(final Dungeon dungeon, final int z, final int w) {
	// Pre-Pass
	final DungeonObjectList objects = DungeonDiver4.getApplication().getObjects();
	final AbstractDungeonObject pass1FillBottom = PreferencesManager.getEditorDefaultFill();
	final AbstractDungeonObject pass1FillTop = new Empty();
	final AbstractDungeonObject[] withoutRuleSets = objects.getAllObjectsWithoutRuleSets();
	final AbstractDungeonObject[] withRuleSets = objects.getAllObjectsWithRuleSets();
	RandomRange r = null;
	int x, y, e;
	// Pass 1
	this.fillFloor(pass1FillBottom, pass1FillTop, z);
	...
	if (withoutRuleSets != null) {
		...
		if (objectsWithoutPrerequisites != null && objectsWithoutPrerequisites.length > 0) {
			...
			for (y = 0; y < rows; y++) {
			    final AbstractDungeonObject placeObj = objectsWithoutPrerequisites[r.generate()];
			    final boolean okay = placeObj.shouldGenerateObject(dungeon, y, x, z, w, e);
			    if (okay) {
				this.setCell(objects.getNewInstanceByName(placeObj.getName()), y, x, z, e);
				placeObj.editorGenerateHook(y, x, z);
			    }
	    ...
	    // Pass 3
	    for (int layer = 0; layer < DungeonConstants.LAYER_COUNT; layer++) {
		final AbstractDungeonObject[] requiredObjects = DungeonObjectList.getAllRequiredSubset(withoutRuleSets,
			layer);
		if (requiredObjects != null) {
		    final RandomRange row = new RandomRange(0, this.getRows() - 1);
		    final RandomRange column = new RandomRange(0, this.getColumns() - 1);
	...
	if (withRuleSets != null) {
		...
		if (objectsWithoutPrerequisites != null) {
			...
			for (y = 0; y < rows; y++) {
			    final AbstractDungeonObject placeObj = objectsWithoutPrerequisites[r.generate()];
			    final boolean okay = placeObj.getRuleSet().shouldGenerateObject(dungeon, y, x, z, w, e);
			    if (okay) {
				this.setCell(objects.getNewInstanceByName(placeObj.getName()), y, x, z, e);
				placeObj.editorGenerateHook(y, x, z);
			    }
	    ...
	    // Pass 5
	    for (int layer = 0; layer < DungeonConstants.LAYER_COUNT; layer++) {
		final AbstractDungeonObject[] requiredObjects = DungeonObjectList.getAllRequiredSubset(withRuleSets,
			layer);
		if (requiredObjects != null) {
		    final RandomRange row = new RandomRange(0, this.getRows() - 1);
		    final RandomRange column = new RandomRange(0, this.getColumns() - 1);
			...
			randomColumn = column.generate();
			final AbstractDungeonObject currObj = requiredObjects[x];
			final int min = currObj.getRuleSet().getMinimumRequiredQuantity(dungeon);
			int max = currObj.getRuleSet().getMaximumRequiredQuantity(dungeon);
			if (max == RandomGenerationRule.NO_LIMIT) {
			    // Maximum undefined, so define it relative to this
			    // dungeon
			    max = this.getRows() * this.getColumns() / 10;
			...
			for (y = 0; y < generateHowMany; y++) {
			    randomRow = row.generate();
			    randomColumn = column.generate();
			    if (currObj.getRuleSet().shouldGenerateObject(dungeon, randomColumn, randomRow, z, w,
				    layer)) {
				this.setCell(objects.getNewInstanceByName(currObj.getName()), randomColumn, randomRow,
					z, layer);
				currObj.editorGenerateHook(y, x, z);
			    } else {
				while (!currObj.getRuleSet().shouldGenerateObject(dungeon, randomRow, randomColumn, z,
					w, layer)) {
				    randomRow = row.generate();
				    randomColumn = column.generate();
				}
    ...
    public void fillFloorAndLayer(final AbstractDungeonObject fillWith, final int z, final int e) {
	int x, y;
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		this.setCell(fillWith, y, x, z, e);
    ...
    public void fillLayerRandomly(final Dungeon dungeon, final int w, final int e) {
	for (int z = 0; z < this.getFloors(); z++) {
	    this.fillFloorAndLayerRandomly(dungeon, z, w, e);
	}
    }

    public void fillLayerRandomlyCustom(final Dungeon dungeon, final int w, final int e) {
	for (int z = 0; z < this.getFloors(); z++) {
	    this.fillFloorAndLayerRandomlyCustom(dungeon, z, w, e);
	}
    }

    public void fillFloorAndLayerRandomly(final Dungeon dungeon, final int z, final int w, final int layer) {
	// Pre-Pass
	final DungeonObjectList objects = DungeonDiver4.getApplication().getObjects();
	final AbstractDungeonObject pass1Fill = PreferencesManager.getEditorDefaultFill(layer);
	RandomRange r = null;
	int x, y;
	// Pass 1
	...
	if (objectsWithoutPrerequisites != null) {
		...
		for (y = 0; y < rows; y++) {
		    final AbstractDungeonObject placeObj = objectsWithoutPrerequisites[r.generate()];
		    final boolean okay = placeObj.shouldGenerateObject(dungeon, x, y, z, w, layer);
		    if (okay) {
    ...
    public void fillFloorAndLayerRandomlyCustom(final Dungeon dungeon, final int z, final int w, final int layer) {
	// Pre-Pass
	final DungeonObjectList objects = DungeonDiver4.getApplication().getObjects();
	final AbstractDungeonObject pass1Fill = PreferencesManager.getEditorDefaultFill(layer);
	final AbstractDungeonObject[] withoutRuleSets = objects.getAllObjectsWithoutRuleSets();
	...
	if (withoutRuleSets != null) {
		...
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
			final AbstractDungeonObject placeObj = objectsWithoutPrerequisites[r.generate()];
			final boolean okay = placeObj.shouldGenerateObject(dungeon, y, x, z, w, layer);
			if (okay) {
			    this.setCell(objects.getNewInstanceByName(placeObj.getName()), y, x, z, layer);
			    placeObj.editorGenerateHook(y, x, z);
			}
	...
	if (withRuleSets != null) {
		...
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
			final AbstractDungeonObject placeObj = objectsWithoutPrerequisites[r.generate()];
			final boolean okay = placeObj.getRuleSet().shouldGenerateObject(dungeon, y, x, z, w, layer);
			if (okay) {
			    this.setCell(objects.getNewInstanceByName(placeObj.getName()), y, x, z, layer);
			    placeObj.editorGenerateHook(y, x, z);
			}
	    ...
	    // Pass 5
	    final AbstractDungeonObject[] requiredObjects = DungeonObjectList.getAllRequiredSubset(withRuleSets, layer);
	    if (requiredObjects != null) {
		final RandomRange row = new RandomRange(0, this.getRows() - 1);
		final RandomRange column = new RandomRange(0, this.getColumns() - 1);
		int randomColumn, randomRow;
		for (x = 0; x < requiredObjects.length; x++) {
			...
			randomRow = row.generate();
			randomColumn = column.generate();
			if (currObj.getRuleSet().shouldGenerateObject(dungeon, randomColumn, randomRow, z, w, layer)) {
			    this.setCell(objects.getNewInstanceByName(currObj.getName()), randomColumn, randomRow, z,
				    layer);
			    currObj.editorGenerateHook(y, x, z);
			} else {
			    while (!currObj.getRuleSet().shouldGenerateObject(dungeon, randomRow, randomColumn, z, w,
				    layer)) {
				randomRow = row.generate();
				randomColumn = column.generate();
			    }
    ...
    public void fillRandomlyInBattle(final Dungeon map, final int w, final AbstractDungeonObject pass1FillBottom,
    ...
    private void fillFloorRandomlyInBattle(final Dungeon map, final int z, final int w,
	    final AbstractDungeonObject pass1FillBottom, final AbstractDungeonObject pass1FillTop) {
	// Pre-Pass
	...
	final int rows = this.getRows();
	for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
	    final AbstractDungeonObject[] objectsWithoutPrerequisites = objects.getAllNotRequiredInBattle(e);
	    if (objectsWithoutPrerequisites != null) {
		r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
	...
	// Pass 3
	for (int layer = 0; layer < DungeonConstants.LAYER_COUNT; layer++) {
	    final AbstractDungeonObject[] requiredObjects = objects.getAllRequiredInBattle(layer);
	    if (requiredObjects != null) {
		final RandomRange row = new RandomRange(0, this.getRows() - 1);
		final RandomRange column = new RandomRange(0, this.getColumns() - 1);
		int randomColumn, randomRow;
	...
	final AbstractDungeonObject bottom = PreferencesManager.getEditorDefaultFill();
	...
	randomRow = row.generate();
	randomColumn = column.generate();
	AbstractDungeonObject currObj = this.getCell(randomRow, randomColumn, z, DungeonConstants.LAYER_OBJECT);
	randomRow = row.generate();
	randomColumn = column.generate();
	if (!currObj.isSolid()) {
		...
		randomRow = row.generate();
		randomColumn = column.generate();
		currObj = this.getCell(randomRow, randomColumn, z, DungeonConstants.LAYER_OBJECT);
	    }
	    this.setStartRow(randomRow);
	    this.setStartColumn(randomColumn);
	    this.setStartFloor(z);
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
			...
			if (mo.getName().equals(o.getName())) {
			    this.setCell(decayTo, y, x, z, DungeonConstants.LAYER_OBJECT);
			}
		    }
		}
	    }
	...
	final AbstractDungeonObject[][][] tempStorage = new AbstractDungeonObject[2 * r + 1][2 * r
		+ 1][DungeonConstants.LAYER_COUNT];
	try {
	    for (u = x - r; u <= x + r; u++) {
		for (v = y - r; v <= y + r; v++) {
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean reactsToIce = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT)
			    .isOfType(TypeConstants.TYPE_REACTS_TO_ICE);
		    if (reactsToIce) {
			final AbstractDungeonObject there = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT);
			if (there.getClass() == BarrierGenerator.class) {
			    // Freeze the generator
			    this.setCell(new IcedBarrierGenerator(), u, v, z, DungeonConstants.LAYER_OBJECT);
			} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    // Freeze the ground
		    this.setCell(new Ice(), u, v, z, DungeonConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
	    }
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean reactsToFire = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT)
			    .isOfType(TypeConstants.TYPE_REACTS_TO_FIRE);
		    if (reactsToFire) {
			final AbstractDungeonObject there = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT);
			if (there.getClass() == BarrierGenerator.class) {
			    // Enrage the generator
			    this.setCell(new EnragedBarrierGenerator(), u, v, z, DungeonConstants.LAYER_OBJECT);
			} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    // Burn the ground
		    this.setCell(new Dirt(), u, v, z, DungeonConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
	    }
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean reactsToPoison = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT)
			    .isOfType(TypeConstants.TYPE_REACTS_TO_POISON);
		    if (reactsToPoison) {
			final AbstractDungeonObject there = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT);
			if (there.getClass() == BarrierGenerator.class) {
			    // Weaken the generator
			    this.setCell(new PoisonedBarrierGenerator(), u, v, z, DungeonConstants.LAYER_OBJECT);
			} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    // Poison the ground
		    this.setCell(new Slime(), u, v, z, DungeonConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
	    }
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean reactsToShock = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT)
			    .isOfType(TypeConstants.TYPE_REACTS_TO_SHOCK);
		    if (reactsToShock) {
			final AbstractDungeonObject there = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT);
			if (there.getClass() == BarrierGenerator.class) {
			    // Shock the generator
			    this.setCell(new ShockedBarrierGenerator(), u, v, z, DungeonConstants.LAYER_OBJECT);
			} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    // Shock the ground
		    this.setCell(new ForceField(), u, v, z, DungeonConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
	    }
	...
	final AbstractDungeonObject[][][] preShuffle = new AbstractDungeonObject[2 * r + 1][2 * r
		+ 1][DungeonConstants.LAYER_COUNT];
	...
	// Do the shuffle
	final AbstractDungeonObject[][][] postShuffle = LayeredTower.shuffleObjects(preShuffle, r, px - (x - r),
		py - (y - r), needsFix);
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean isEmpty = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT)
			...
			if (chance == 1) {
			    // Grow a crevasse
			    this.setCell(new Crevasse(), u, v, z, DungeonConstants.LAYER_OBJECT);
			}
		    }
		    final boolean isBreakable = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT)
			    .isOfType(TypeConstants.TYPE_BREAKABLE_WALL);
		    if (isBreakable) {
			// Destroy the wall
			this.setCell(new Empty(), u, v, z, DungeonConstants.LAYER_OBJECT);
		    }
		    final boolean isWall = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT)
			    .isOfType(TypeConstants.TYPE_PLAIN_WALL);
		    if (isWall) {
			// Crack the wall
			this.setCell(new CrackedWall(), u, v, z, DungeonConstants.LAYER_OBJECT);
		    }
		    final boolean isCharacter = this.getCell(u, v, z, DungeonConstants.LAYER_OBJECT)
    ...
    public void warpObject(final AbstractDungeonObject mo, final int x, final int y, final int z, final int l) {
	final RandomRange row = new RandomRange(0, this.getRows() - 1);
	final RandomRange column = new RandomRange(0, this.getColumns() - 1);
	int randomColumn, randomRow;
	randomRow = row.generate();
	randomColumn = column.generate();
	final AbstractDungeonObject currObj = this.getCell(randomRow, randomColumn, z, DungeonConstants.LAYER_OBJECT);
	if (!currObj.isSolid()) {
	    this.setCell(new Empty(), x, y, z, l);
	    this.setCell(mo, randomRow, randomColumn, z, DungeonConstants.LAYER_OBJECT);
	} else {
	    while (currObj.isSolid()) {
		randomRow = row.generate();
		randomColumn = column.generate();
		this.getCell(randomRow, randomColumn, z, DungeonConstants.LAYER_OBJECT);
	    }
	    this.setCell(new Empty(), x, y, z, l);
	    this.setCell(mo, randomRow, randomColumn, z, DungeonConstants.LAYER_OBJECT);
	}
    ...
    private static AbstractDungeonObject[][][] shuffleObjects(final AbstractDungeonObject[][][] preShuffle, final int r,
	    final int opx, final int opy, final boolean needsFix) {
	final AbstractDungeonObject[][][] postShuffle = new AbstractDungeonObject[2 * r + 1][2 * r
		+ 1][DungeonConstants.LAYER_COUNT];
	...
	final LayeredTower lt = new LayeredTower(dungeonSizeX, dungeonSizeY, dungeonSizeZ);
	for (x = 0; x < lt.getColumns(); x++) {
	    for (y = 0; y < lt.getRows(); y++) {
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
			lt.setCell(DungeonDiver4.getApplication().getObjects().readDungeonObject(reader,
				FormatConstants.DUNGEON_FORMAT_1), y, x, z, e);
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
			this.savedTowerState.getCell(y, x, z, e).writeDungeonObject(writer);
		    }
		}
	    }
	}
	...
	this.savedTowerState = new LowLevelADODataStore(sizeY, sizeX, sizeZ, DungeonConstants.LAYER_COUNT);
	for (x = 0; x < sizeY; x++) {
	    for (y = 0; y < sizeX; y++) {
		for (z = 0; z < sizeZ; z++) {
		    for (e = 0; e < DungeonConstants.LAYER_COUNT; e++) {
			this.savedTowerState.setCell(
				DungeonDiver4.getApplication().getObjects().readDungeonObject(reader, formatVersion), y,
				x, z, e);
		    }
		}
	    }
