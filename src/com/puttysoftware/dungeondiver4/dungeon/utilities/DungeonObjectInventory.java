...
public final class DungeonObjectInventory implements Cloneable {
	...
	final AbstractDungeonObject[] invobj = list.getAllInventoryableObjectsMinusSpecial();
	...
	if (!DungeonObjectInventory.isBoots(mo) && !DungeonObjectInventory.isAmulet(mo)) {
	    if (!DungeonObjectInventory.isBow(mo)) {
    ...
    public void use(final AbstractDungeonObject mo, final int x, final int y, final int z) {
	...
	final DungeonObjectList list = DungeonDiver4.getApplication().getObjects();
	final AbstractDungeonObject[] objects = list.getAllInventoryableObjectsMinusSpecial();
	...
	if (!this.boots.getName().equals(DungeonObjectInventory.DEFAULT_BOOTS.getName())) {
	    this.boots.stepAction();
	}
	if (!this.amulet.getName().equals(DungeonObjectInventory.DEFAULT_AMULET.getName())) {
    ...
    private void setOtherUses(final AbstractDungeonObject mo, final int newUses) {
	final int loc = this.indexOf(mo);
    ...
    public static DungeonObjectInventory readInventory(final XDataReader reader, final int formatVersion)
	    throws IOException {
	...
	i.boots = (AbstractBoots) objects.readDungeonObject(reader, formatVersion);
	if (i.boots == null) {
	    i.boots = DungeonObjectInventory.DEFAULT_BOOTS;
	}
	i.amulet = (AbstractAmulet) objects.readDungeonObject(reader, formatVersion);
	if (i.amulet == null) {
