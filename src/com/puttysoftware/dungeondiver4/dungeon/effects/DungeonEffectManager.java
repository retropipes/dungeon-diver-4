...
public class DungeonEffectManager {
	...
	this.activeEffects[DungeonEffectConstants.EFFECT_GHOSTLY] = new Ghostly(0);
	this.activeEffects[DungeonEffectConstants.EFFECT_POISONOUS] = new Poisonous(0);
	this.activeEffects[DungeonEffectConstants.EFFECT_COUNTER_POISONED] = new CounterPoisoned(0);
	this.activeEffects[DungeonEffectConstants.EFFECT_TRUE_SIGHT] = new TrueSight(0);
	// Create GUI
	...
	for (int x = 0; x < DungeonEffectManager.NUM_EFFECTS; x++) {
		...
		if (!this.activeEffects[x].isActive()) {
		    DungeonDiver4.getApplication().showMessage("You feel normal again.");
		    // Clear effect grid
		    this.clearGridEntry(x);
		    // Pack
		    DungeonDiver4.getApplication().getGameManager().getOutputFrame().pack();
		}
	    }
	}
    }
