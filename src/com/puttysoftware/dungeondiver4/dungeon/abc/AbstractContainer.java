...
public abstract class AbstractContainer extends AbstractLock {
    ...
    protected AbstractContainer(final AbstractSingleKey mgk, final AbstractDungeonObject insideObject) {
	super(mgk);
	...
	if (!this.inside.getName().equals("Empty")) {
	    DungeonDiver4.getApplication().showMessage(this.getName() + ": Contains " + this.inside.getName());
	} else {
	    DungeonDiver4.getApplication().showMessage(this.getName() + ": Contains Nothing");
	}
    }
