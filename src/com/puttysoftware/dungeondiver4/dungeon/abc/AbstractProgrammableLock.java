...
public abstract class AbstractProgrammableLock extends AbstractSingleLock {
	...
	if (!app.getGameManager().isEffectActive(DungeonEffectConstants.EFFECT_GHOSTLY)
	    ...
	    DungeonDiver4.getApplication().getGameManager().addToScore(AbstractLock.SCORE_UNLOCK);
	} else {
	    SoundManager.playSound(SoundConstants.SOUND_WALK);
	}
    }
	...
	if (this.isConditionallyDirectionallySolid(ie, dirX, dirY, inv)) {
	    if (this.getKey() == AbstractProgrammableLock.SIGNAL) {
		DungeonDiver4.getApplication().showMessage("You need a Crystal");
	    } else {
		DungeonDiver4.getApplication().showMessage("You need a " + this.getKey().getName());
	    }
	}
	...
	final AbstractDungeonObject[] tempKeys = objects.getAllProgrammableKeys();
	final String[] keyNames = new String[tempKeyNames.length + 1];
	final AbstractDungeonObject[] keys = new AbstractDungeonObject[tempKeys.length + 1];
    ...
    protected AbstractDungeonObject readDungeonObjectHook(final XDataReader reader, final int formatVersion)
	    throws IOException {
	final AbstractDungeonObject o = DungeonDiver4.getApplication().getObjects().readDungeonObject(reader,
		formatVersion);
	if (o == null) {
