...
public abstract class AbstractDungeonObject extends CloneableObject implements TypeConstants, RandomGenerationRule {
    // Properties
    ...
    public AbstractDungeonObject(final boolean isSolid, final boolean isPushable, final boolean doesAcceptPushInto,
    ...
    public AbstractDungeonObject(final boolean isSolid, final boolean canBeInventoried, final boolean sightBlock) {
	this.sp = new SolidProperties();
	this.sp.setSolid(isSolid);
	this.pushable = false;
	this.pushableInto = false;
    ...
    public AbstractDungeonObject(final boolean isSolid, final boolean isUsable, final int newUses,
     ...
     * @return
     */
    public boolean hasFrictionConditionally(final DungeonObjectInventory inv, final boolean moving) {
     ...
     * @param z
     */
    public void pushIntoAction(final DungeonObjectInventory inv, final AbstractDungeonObject pushed, final int x,
     ...
     * @param z
     */
    public void pushOutAction(final DungeonObjectInventory inv, final AbstractDungeonObject pushed, final int x,
	    final int y, final int z) {
	// Do nothing
    }
     ...
     * @param pushY
     */
    public void pushFailedAction(final DungeonObjectInventory inv, final int x, final int y, final int pushX,
	    final int pushY) {
     ...
     * @param z
     */
    public void pullIntoAction(final DungeonObjectInventory inv, final AbstractDungeonObject pulled, final int x,
	    final int y, final int z) {
	// Do nothing
    }
     ...
     * @param z
     */
    public void pullOutAction(final DungeonObjectInventory inv, final AbstractDungeonObject pulled, final int x,
	    final int y, final int z) {
	// Do nothing
    }
     ...
     * @param pullY
     */
    public void pullFailedAction(final DungeonObjectInventory inv, final int x, final int y, final int pullX,
	    final int pullY) {
	SoundManager.playSound(SoundConstants.SOUND_ACTION_FAILED);
	DungeonDiver4.getApplication().getGameManager().keepNextMessage();
     ...
     * @return
     */
    public AbstractDungeonObject editorRenderHook(final int x, final int y, final int z) {
	return this;
    }
    ...
    @Override
    public boolean shouldGenerateObjectInBattle(final Dungeon dungeon, final int row, final int col, final int floor,
	    final int level, final int layer) {
	if (!this.enabledInBattle()) {
    ...
    public final AbstractDungeonObject readDungeonObjectV1(final XDataReader reader, final String ident)
	    throws IOException {
	if (ident.equals(this.getIdentifier())) {
		...
		this.saved = DungeonDiver4.getApplication().getObjects().readSavedDungeonObject(reader, savedIdent,
			FormatConstants.DUNGEON_FORMAT_1);
	    }
	    reader.readOpeningGroup(AbstractDungeonObject.OBJECT_SETTINGS_GROUP);
	    final int cc = this.getCustomFormat();
	    if (cc == AbstractDungeonObject.CUSTOM_FORMAT_MANUAL_OVERRIDE) {
		return this.readDungeonObjectHook(reader, FormatConstants.DUNGEON_FORMAT_1);
	    } else {
		for (int x = 0; x < cc; x++) {
	    ...
	    reader.readClosingGroup(AbstractDungeonObject.OBJECT_SETTINGS_GROUP);
	    return this;
	} else {
	    return null;
	}
