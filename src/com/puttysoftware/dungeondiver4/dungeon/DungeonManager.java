...
public class DungeonManager implements OpenFilesHandler {
    // Fields
	...
	if (!this.loaded) {
		...
		this.scoresFileName = DungeonManager.getNameWithoutExtension(file.getName());
	...
	if (saved) {
		...
		if (ff.getDescription().equals(xmf.getDescription())) {
		    PreferencesManager.setLastFilterUsedOpen(PreferencesManager.FILTER_DUNGEON);
		} else {
	...
	if (saved) {
		...
		if (extension.equals(Extension.getGameExtension())) {
			...
			if (!okay) {
			    DungeonDiver4.logError(new IOException("Cannot create game folder!"));
			}
		    }
		    try {
			FileUtilities.copyFile(file, new File(DungeonManager.getGameDirectory() + file.getName()));
		    } catch (final IOException ioe) {
			DungeonDiver4.logError(ioe);
		    }
		    DungeonManager.loadFile(filename, false, true);
		} else {
		    CommonDialogs.showDialog(
	...
	if (app.getMode() == Application.STATUS_GAME) {
	    ...
	    if (this.lastUsedDungeonFile != null && !this.lastUsedDungeonFile.equals("")) {
