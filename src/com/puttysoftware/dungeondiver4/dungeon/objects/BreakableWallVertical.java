...
public class BreakableWallVertical extends AbstractWall {
	...
	final int dirZ = DungeonDiver4.getApplication().getDungeonManager().getDungeon().getPlayerLocationZ();
	...
	// Break up
	DungeonDiver4.getApplication().getGameManager().morph(new Empty(), dirX, dirY, dirZ,
		DungeonConstants.LAYER_OBJECT);
	final Dungeon m = DungeonDiver4.getApplication().getDungeonManager().getDungeon();
	final AbstractDungeonObject above = m.getCell(dirX, dirY - 1, dirZ, DungeonConstants.LAYER_OBJECT);
	try {
	...
	try {
	    final AbstractDungeonObject below = m.getCell(dirX, dirY + 1, dirZ, DungeonConstants.LAYER_OBJECT);
