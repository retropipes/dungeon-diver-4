...
public class Pit extends StairsDown {
	...
	return this.searchNestedPits(dirX, dirY,
		DungeonDiver4.getApplication().getDungeonManager().getDungeon().getPlayerLocationZ() - 1, inv);
    }

    private boolean searchNestedPits(final int dirX, final int dirY, final int floor,
	...
	if (app.getGameManager().doesFloorExist(floor)) {
	    final AbstractDungeonObject obj = app.getDungeonManager().getDungeon().getCell(dirX, dirY, floor,
	...
	try {
	    ...
	    SoundManager.playSound(SoundConstants.SOUND_INTO_PIT);
	    DungeonDiver4.getApplication().getDungeonManager().getDungeon().setCell(new Empty(), x, y, z,
		    DungeonConstants.LAYER_OBJECT);
	}
    }
