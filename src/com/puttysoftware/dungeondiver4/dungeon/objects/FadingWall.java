...
public class FadingWall extends AbstractWall {
	...
	final int tx = app.getDungeonManager().getDungeon().getPlayerLocationX();
	final int ty = app.getDungeonManager().getDungeon().getPlayerLocationY();
	final int pz = app.getDungeonManager().getDungeon().getPlayerLocationZ();
	scanResult = app.getDungeonManager().getDungeon().radialScan(dirX, dirY, FadingWall.SCAN_LIMIT, tx, ty);
	if (scanResult) {
