...
public class MetalButton extends AbstractField {
	...
	if (inv.isItemThere(this.getKey())) {
	    ...
	    final AbstractDungeonObject there = app.getDungeonManager().getDungeonObject(this.getTargetRow(),
	...
	DungeonDiver4.getApplication().showMessage(this.getName() + ": Target (" + (this.targetCol + 1) + ","
		+ (this.targetRow + 1) + "," + (this.targetFloor + 1) + "," + (this.targetLevel + 1) + ")");
    }

    @Override
    public AbstractDungeonObject editorPropertiesHook() {
	return DungeonDiver4.getApplication().getEditor().editMetalButtonTarget();
    }
