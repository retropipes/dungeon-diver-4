...
public class MapBattleLogic extends AbstractBattle {
	...
	DungeonDiver4.getApplication().getGameManager().addToScore(m.getExperience() + m.getGold());
	// Level Up Check
	if (playerCharacter.checkLevelUp()) {
	    playerCharacter.levelUp();
	    DungeonDiver4.getApplication().getGameManager().keepNextMessage();
	    DungeonDiver4.getApplication().showMessage("You reached level " + playerCharacter.getLevel() + ".");
	}
    }
	...
	if (!this.resultDoneAlready) {
		...
		DungeonDiver4.getApplication().getGameManager().addToScore(Math.max(1,
	...
	if (this.damage == 0) {
		...
		if (this.damage < 0) {
		    displayDamageString = displayDamagePrefix + activeName + " tries to hit " + enemyName
			    + ", but is RIPOSTED for " + riposteDamageString + " damage!";
		    ...
		    displayDamageString = displayDamagePrefix + activeName + " hits " + enemyName + " for "
			    + damageString + " damage!";
	...
	final RandomRange randX = new RandomRange(0, this.bd.getBattleDungeon().getRows() - 1);
	final RandomRange randY = new RandomRange(0, this.bd.getBattleDungeon().getColumns() - 1);
	int rx, ry;
	// Set Character Locations
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlers()[x].isActive() && this.bd.getBattlers()[x].getTemplate().getX() == -1
		    ...
		    rx = randX.generate();
		    ry = randY.generate();
		    AbstractDungeonObject obj = this.bd.getBattleDungeon().getBattleCell(rx, ry);
		    ...
		    this.bd.getBattleDungeon().setBattleCell(this.bd.getBattlers()[x], rx, ry);
		}
	    }
	}
    }
	...
	if (res != -1) {
	    ...
	    // AI Check
	    if (this.bd.getActiveCharacter().getTemplate().hasAI() && !DungeonDiver4.inDebugMode()) {
		// Run AI
		this.waitForAI();
	...
	if (hit != null && !hit.getTemplate().isAlive()) {
	    ...
	    this.bd.getBattleDungeon().setBattleCell(new Empty(), hit.getX(), hit.getY());
	}
	// Check result
	...
	if (next != null && nextGround != null && currGround != null) {
		...
		if (useAP && this.getActiveActionCounter() >= AbstractDungeonObject.getBattleAPCost() || !useAP) {
		    // Move
		    ...
		    this.decrementActiveActionCounterBy(AbstractDungeonObject.getBattleAPCost());
		...
		if (next.isOfType(TypeConstants.TYPE_BATTLE_CHARACTER)) {
			...
			if (!enemy.isAlive()) {
			    ...
			    bc.deactivate();
			    // Remove character from battle
			    this.bd.getBattleDungeon().setBattleCell(new Empty(), bc.getX(), bc.getY());
			}
			// Handle self death
			if (!active.getTemplate().isAlive()) {
			    ...
			    active.deactivate();
			    // Remove character from battle
			    this.bd.getBattleDungeon().setBattleCell(new Empty(), active.getX(), active.getY());
			    // End turn
			    this.endTurn();
			}
		    } else {
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (!active.isAlive()) {
		    ...
		    // Remove character from battle
		    this.bd.getBattleDungeon().setBattleCell(new Empty(), this.bd.getBattlers()[x].getX(),
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlerAIContexts()[x] != null
			...
			&& !DungeonDiver4.inDebugMode() && this.bd.getBattlers()[x].isActive()
