...
class MapBattleGUI {
	...
	this.battleFrame.setJMenuBar(DungeonDiver4.getApplication().getMenuManager().getMainMenuBar());
    }
	...
	if (this.battleFrame.isVisible()) {
		...
		for (y = yView; y <= ylView; y++) {
		    ...
		    yFix = y - yView;
		    try {
			final BufferedImageIcon icon1 = bd.getBattleDungeon().getBattleGround(y, x).battleRenderHook();
			final BufferedImageIcon icon2 = bd.getBattleDungeon().getBattleCell(y, x).battleRenderHook();
	...
	if (this.battleFrame.isVisible()) {
		...
		xFix = y - xView;
		yFix = x - yView;
		final BufferedImageIcon icon1 = bd.getBattleDungeon().getBattleGround(x, y).battleRenderHook();
		final BufferedImageIcon icon2 = bd.getBattleDungeon().getBattleCell(x, y).battleRenderHook();
	...
	for (int x = 0; x < MapBattleViewingWindowManager.getViewingWindowSize(); x++) {
		...
		final AbstractDungeonObject dark = new Darkness().gameRenderHook(y, x, 0);
	...
	private void handleMovement(final KeyEvent e) {
		...
		final AbstractBattle bl = DungeonDiver4.getApplication().getBattle();
