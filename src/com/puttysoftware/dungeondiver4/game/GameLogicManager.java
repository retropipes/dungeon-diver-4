...
public class GameLogicManager {
    // Fields
    private AbstractDungeonObject savedDungeonObject, previousSavedDungeonObject;
	...
	final Dungeon m = DungeonDiver4.getApplication().getDungeonManager().getDungeon();
	boolean b = false;
	...
	final AbstractDungeonObject acted = DungeonDiver4.getApplication().getDungeonManager().getDungeonObject(x, y, z,
	...
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
	    m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(), floor, DungeonConstants.LAYER_OBJECT);
	    return true;
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    return false;
	} catch (final NullPointerException np) {
	...
	do {
		...
		try {
		    if (this.checkSolid(this.savedDungeonObject, below, nextBelow, nextAbove)) {
	...
	try {
	    nextBelow = m.getCell(px + fX, py + fY, pz, DungeonConstants.LAYER_GROUND);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    nextBelow = new Empty();
	}
	try {
	    nextAbove = m.getCell(px + fX, py + fY, pz, DungeonConstants.LAYER_OBJECT);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    nextAbove = new Wall();
	}
	try {
	    nextNextBelow = m.getCell(px + 2 * fX, py + 2 * fY, pz, DungeonConstants.LAYER_GROUND);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    nextNextBelow = new Empty();
	}
	try {
	    nextNextAbove = m.getCell(px + 2 * fX, py + 2 * fY, pz, DungeonConstants.LAYER_OBJECT);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	...
	if (acted.isPushable()) {
		...
		m.setCell(this.savedDungeonObject, px, py, pz, DungeonConstants.LAYER_OBJECT);
		...
		this.savedDungeonObject = m.getCell(px, py, pz, DungeonConstants.LAYER_OBJECT);
		...
		if (groundInto.overridesDefaultPostMove()) {
		    ...
		    if (!this.savedDungeonObject.isOfType(TypeConstants.TYPE_PASS_THROUGH)) {
			this.savedDungeonObject.postMoveAction(false, px, py, this.getObjectInventory());
		    }
		} else {
		    this.savedDungeonObject.postMoveAction(false, px, py, this.getObjectInventory());
		}
	    ...
	    this.fireMoveFailedActions(px + fX, py + fY, this.savedDungeonObject, below, nextBelow, nextAbove);
	    this.fireStepActions();
	    this.decayEffects();
	}
	...
	try {
	    previousBelow = m.getCell(px - fX, py - fY, pz, DungeonConstants.LAYER_GROUND);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	...
	if (acted.isPullable() && this.isPullInProgress()) {
	    if (!this.checkPull(fX, fY, pullX, pullY, acted, previousBelow, below, this.savedDungeonObject)) {
    ...
    private boolean checkLoopCondition(final boolean proceed, final AbstractDungeonObject below,
	...
	if (this.getObjectInventory().isItemThere(new SlipperyBoots())) {
	    return proceed && this.checkSolid(this.savedDungeonObject, below, nextBelow, nextAbove);
	} else {
	    return proceed && !nextBelow.hasFrictionConditionally(this.getObjectInventory(), false)
		    && this.checkSolid(this.savedDungeonObject, below, nextBelow, nextAbove);
	}
    }

    private boolean checkSolid(final AbstractDungeonObject inside, final AbstractDungeonObject below,
	    final AbstractDungeonObject nextBelow, final AbstractDungeonObject nextAbove) {
    ...
    private void fireMoveFailedActions(final int x, final int y, final AbstractDungeonObject inside,
    ...
    private boolean checkPush(final int x, final int y, final int pushX, final int pushY,
	    final AbstractDungeonObject acted, final AbstractDungeonObject nextBelow,
	    final AbstractDungeonObject nextNextBelow, final AbstractDungeonObject nextNextAbove) {
	final Application app = DungeonDiver4.getApplication();
	final Dungeon m = app.getDungeonManager().getDungeon();
	final int px = m.getPlayerLocationX();
	final int py = m.getPlayerLocationY();
    ...
    private boolean checkPull(final int x, final int y, final int pullX, final int pullY,
	    final AbstractDungeonObject acted, final AbstractDungeonObject previousBelow,
	...
	AbstractDungeonObject there = mm.getDungeonObject(m.getPlayerLocationX() + cumX, m.getPlayerLocationY() + cumY,
	...
	if (there != null) {
		...
		cumPushX += xInc;
		cumPushY += yInc;
		there = mm.getDungeonObject(m.getPlayerLocationX() + cumX, m.getPlayerLocationY() + cumY,
			m.getPlayerLocationZ(), DungeonConstants.LAYER_GROUND);
		if (there == null) {
		    break;
		}
    ...
    private void movePushedObjectPosition(final int x, final int y, final int pushX, final int pushY,
	    final AbstractMovableObject o, final AbstractDungeonObject g) {
	final Application app = DungeonDiver4.getApplication();
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
	...
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
	    if (!m.getCell(x, y, z, DungeonConstants.LAYER_OBJECT).isConditionallySolid(this.getObjectInventory())) {
		final AbstractDungeonObject saved = m.getCell(x, y, z, DungeonConstants.LAYER_OBJECT);
	...
	try {
	    ...
	    final AbstractDungeonObject below = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(),
		    m.getPlayerLocationZ(), DungeonConstants.LAYER_GROUND);
	    final AbstractDungeonObject nextBelow = m.getCell(m.getPlayerLocationX() + x, m.getPlayerLocationY() + y,
		    m.getPlayerLocationZ(), DungeonConstants.LAYER_GROUND);
	    final AbstractDungeonObject nextAbove = m.getCell(m.getPlayerLocationX() + x, m.getPlayerLocationY() + y,
		    m.getPlayerLocationZ(), DungeonConstants.LAYER_OBJECT);
	    return this.checkSolid(this.savedDungeonObject, below, nextBelow, nextAbove);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    return false;
	}
    }
	...
	try {
	    ...
	    return this.checkSolidAbsolute(this.savedDungeonObject, below, nextBelow, nextAbove);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    return false;
	}
    }

    private boolean checkSolidAbsolute(final AbstractDungeonObject inside, final AbstractDungeonObject below,
	    final AbstractDungeonObject nextBelow, final AbstractDungeonObject nextAbove) {
	final boolean insideSolid = inside.isConditionallySolid(this.getObjectInventory());
	final boolean belowSolid = below.isConditionallySolid(this.getObjectInventory());
	final boolean nextBelowSolid = nextBelow.isConditionallySolid(this.getObjectInventory());
	...
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
	    m.getCell(x, y, z, DungeonConstants.LAYER_OBJECT).preMoveAction(true, x, y, this.getObjectInventory());
	} catch (final ArrayIndexOutOfBoundsException ae) {
	    // Do nothing
	} catch (final NullPointerException np) {
	    // Do nothing
	...
	try {
	    if (!m.getCell(x, y, z, DungeonConstants.LAYER_OBJECT).isConditionallySolid(this.getObjectInventory())) {
		m.setCell(this.savedDungeonObject, m.getPlayerLocationX(), m.getPlayerLocationY(),
			m.getPlayerLocationZ(), DungeonConstants.LAYER_OBJECT);
		...
		this.savedDungeonObject = m.getCell(m.getPlayerLocationX(), m.getPlayerLocationY(),
			m.getPlayerLocationZ(), DungeonConstants.LAYER_OBJECT);
		app.getDungeonManager().setDirty(true);
		this.savedDungeonObject.postMoveAction(false, x, y, this.getObjectInventory());
		final int px = m.getPlayerLocationX();
		final int py = m.getPlayerLocationY();
		final int pz = m.getPlayerLocationZ();
		m.updateVisibleSquares(px, py, pz);
	    ...
	    m.restorePlayerLocation();
	    this.vwMgr.restoreViewingWindow();
	    DungeonDiver4.getApplication().showMessage("Can't go outside the dungeon");
	} catch (final NullPointerException np) {
	    m.restorePlayerLocation();
	    this.vwMgr.restoreViewingWindow();
	    DungeonDiver4.getApplication().showMessage("Can't go outside the dungeon");
    ...
    void redrawOneSquare(final int x, final int y, final boolean useDelay, final AbstractDungeonObject obj4) {
	...
	if (this.actingRemotely) {
	    DungeonDiver4.getApplication().getDungeonManager().getDungeon().setCell(decay, this.remoteCoords[0],
		    this.remoteCoords[1], this.remoteCoords[2], DungeonConstants.LAYER_OBJECT);
	} else {
	...
	if (this.actingRemotely) {
	    DungeonDiver4.getApplication().getDungeonManager().getDungeon().setCell(this.delayedDecayObject,
		    this.remoteCoords[0], this.remoteCoords[1], this.remoteCoords[2], DungeonConstants.LAYER_OBJECT);
	} else {
    ...
    public void morph(final AbstractDungeonObject morphInto, final int x, final int y, final int z, final int e) {
	final Application app = DungeonDiver4.getApplication();
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
    ...
    public void morph(final AbstractDungeonObject morphInto, final int x, final int y, final int z) {
	final Application app = DungeonDiver4.getApplication();
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
    ...
    public void morph(final AbstractDungeonObject morphInto, final int x, final int y, final int z, final String msg) {
	final Application app = DungeonDiver4.getApplication();
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
	    m.setCell(morphInto, x, y, z, morphInto.getLayer());
    ...
    public void morphOther(final AbstractDungeonObject morphInto, final int x, final int y, final int e) {
	final Application app = DungeonDiver4.getApplication();
	final Dungeon m = app.getDungeonManager().getDungeon();
	try {
	...
	try {
	    ...
	    DungeonDiver4.getApplication().showMessage(gameName2 + " on " + gameName1);
    ...
    public void loadGameHook(final XDataReader dungeonFile, final int formatVersion) throws IOException {
	final Application app = DungeonDiver4.getApplication();
	...
	if (app.getDungeonManager().getLoaded()) {
	    ...
	    final int pz = m.getPlayerLocationZ();
	    m.updateVisibleSquares(px, py, pz);
	    CommonDialogs.showTitledDialog(m.getDungeonStartMessage(), m.getDungeonTitle());
