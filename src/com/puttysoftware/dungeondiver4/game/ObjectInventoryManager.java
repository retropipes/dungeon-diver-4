...
class ObjectInventoryManager {
    ...
    void showUseDialog() {
	int x;
	final DungeonObjectList list = DungeonDiver4.getApplication().getObjects();
	...
	final String result = CommonDialogs.showInputDialog("Use which object?", "DungeonDiver4", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
			...
			DungeonDiver4.getApplication().showMessage("That item has no more uses left.");
			this.setUsingAnItem(false);
		    } else {
			DungeonDiver4.getApplication().showMessage("Click to set target");
	...
	final String result = CommonDialogs.showInputDialog("Switch to which bow?", "DungeonDiver4", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
			...
			DungeonDiver4.getApplication().showMessage("That bow is out of arrows!");
		    } else {
			DungeonDiver4.getApplication().showMessage(this.activeBow.getName() + " activated.");
		    }
		    return;
		}
	    }
	...
	if (this.usingAnItem() && app.getMode() == Application.STATUS_GAME) {
	    ...
	    try {
		final AbstractDungeonObject target = m.getCell(x, y, z, DungeonConstants.LAYER_OBJECT);
		final String name = this.objectBeingUsed.getName();
		if ((target.isSolid() || !visible) && name.equals(new TeleportWand().getName())) {
		    this.setUsingAnItem(false);
		    DungeonDiver4.getApplication().showMessage("Can't teleport there");
		}
		if (target.getName().equals(new Player().getName())) {
		    this.setUsingAnItem(false);
		    DungeonDiver4.getApplication().showMessage("Don't aim at yourself!");
		}
		if (!target.isDestroyable() && name.equals(new AnnihilationWand().getName())) {
		    this.setUsingAnItem(false);
		    DungeonDiver4.getApplication().showMessage("Can't destroy that");
		}
		if (!target.isDestroyable() && name.equals(new WallMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    DungeonDiver4.getApplication().showMessage("Can't create a wall there");
		}
		...
		if (!target.getName().equals(new Empty().getName()) && !target.getName().equals(new DarkGem().getName())
		    ...
		    this.setUsingAnItem(false);
		    DungeonDiver4.getApplication().showMessage("Aim at either an empty space or a Dark Gem");
		}
		if (!target.getName().equals(new Empty().getName())
			&& !target.getName().equals(new LightGem().getName())
			&& name.equals(new DarkWand().getName())) {
		    this.setUsingAnItem(false);
		    DungeonDiver4.getApplication().showMessage("Aim at either an empty space or a Light Gem");
		}
    ...
    void readObjectInventory(final XDataReader dungeonFile, final int formatVersion) throws IOException {
	this.objectInv = DungeonObjectInventory.readInventory(dungeonFile, formatVersion);
	this.savedObjectInv = DungeonObjectInventory.readInventory(dungeonFile, formatVersion);
    }

    void writeObjectInventory(final XDataWriter dungeonFile) throws IOException {
