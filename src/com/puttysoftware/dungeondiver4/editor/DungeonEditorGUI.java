...
class DungeonEditorGUI {
	...
	if (w != 0) {
	    // Level Change
	    DungeonDiver4.getApplication().getDungeonManager().getDungeon().switchLevelOffset(w);
	    this.fixLimits();
	    this.setUpGUI();
	}
	...
	this.elMgr.setEditorLocationW(w);
	// Level Change
	DungeonDiver4.getApplication().getDungeonManager().getDungeon().switchLevel(w);
	this.fixLimits();
	this.setUpGUI();
	this.checkMenus(engine);
	this.redrawEditor();
	...
	if (app.getMode() == Application.STATUS_EDITOR) {
	    ...
	    try {
		if (app.getDungeonManager().getDungeon().is3rdDimensionWraparoundEnabled()) {
	...
	for (x = this.evMgr.getViewingWindowLocationX(); x <= this.evMgr.getLowerRightViewingWindowLocationX(); x++) {
		...
		try {
		    final AbstractDungeonObject obj1 = app.getDungeonManager().getDungeon()
			    .getCell(y, x, this.elMgr.getEditorLocationZ(), DungeonConstants.LAYER_GROUND)
		    ...
		    this.drawGrid.setImageCell(
			    ObjectImageManager.getImage(DungeonEditorGUI.EMPTY_VOID.getName(),
				    DungeonEditorGUI.EMPTY_VOID.getBaseID(), ColorConstants.COLOR_NONE,
				    DungeonEditorGUI.EMPTY_VOID.getAttributeID(), ColorConstants.COLOR_NONE),
			    xFix, yFix);
	...
	for (x = this.evMgr.getViewingWindowLocationX(); x <= this.evMgr.getLowerRightViewingWindowLocationX(); x++) {
		...
		try {
		    ...
		    final AbstractDungeonObject obj2 = m
			    .getCell(y, x, this.elMgr.getEditorLocationZ(), DungeonConstants.LAYER_OBJECT)
			    .editorRenderHook(y, x, this.elMgr.getEditorLocationZ());
		    ...
		    if (u == y && v == x) {
			final AbstractDungeonObject obj3 = new Player().editorRenderHook(y, x,
    ...
    void redrawVirtual(final int x, final int y, final AbstractDungeonObject obj3) {
	// Draw the square
	final Application app = DungeonDiver4.getApplication();
	int xFix, yFix;
	...
	try {
	    final AbstractDungeonObject obj1 = app.getDungeonManager().getDungeon()
		    .getCell(y, x, this.elMgr.getEditorLocationZ(), DungeonConstants.LAYER_GROUND)
		    .editorRenderHook(y, x, this.elMgr.getEditorLocationZ());
	    final AbstractDungeonObject obj2 = app.getDungeonManager().getDungeon()
		    .getCell(y, x, this.elMgr.getEditorLocationZ(), DungeonConstants.LAYER_OBJECT)
		    .editorRenderHook(y, x, this.elMgr.getEditorLocationZ());
	    final BufferedImageIcon img1 = ObjectImageManager.getImage(obj1.getName(), obj1.getBaseID(),
		    obj1.getTemplateColor(), obj1.getAttributeID(), obj1.getAttributeTemplateColor());
	    final BufferedImageIcon img2 = ObjectImageManager.getImage(obj2.getName(), obj2.getBaseID(),
		    obj2.getTemplateColor(), obj2.getAttributeID(), obj2.getAttributeTemplateColor());
	    final AbstractDungeonObject obj4 = obj3.editorRenderHook(y, x, this.elMgr.getEditorLocationZ());
	...
	// Fix limits
	final Application app = DungeonDiver4.getApplication();
	if (app.getDungeonManager().getDungeon() != null && this.elMgr != null && this.evMgr != null) {
	    this.elMgr.setLimitsFromDungeon(app.getDungeonManager().getDungeon());
	    this.evMgr.halfOffsetMaximumViewingWindowLocationsFromDungeon(app.getDungeonManager().getDungeon());
	}
    }
	...
	try {
	    final DungeonEditorLogic mel = DungeonDiver4.getApplication().getEditor();
	    final Application app = DungeonDiver4.getApplication();
