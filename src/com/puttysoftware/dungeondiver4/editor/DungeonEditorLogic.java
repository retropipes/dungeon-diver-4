...
public class DungeonEditorLogic {
	...
	final DungeonObjectList objectList = DungeonDiver4.getApplication().getObjects();
	...
	final AbstractDungeonObject instance = app.getObjects().getNewInstanceByName(mo.getName());
	...
	try {
	    ...
	    this.updateUndoHistory(this.savedDungeonObject, gridX, gridY,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationW(),
    ...
    private void autoGenerateTransitions(final AbstractDungeonObject instance, final int gridX, final int gridY) {
    ...
    private void autoGenerateTransitions1(final AbstractDungeonObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	final Application app = DungeonDiver4.getApplication();
	final AbstractDungeonObject[] generatedObjects = app.getObjects().getAllGeneratedTypedObjects();
	final int generatedOffset = app.getObjects().getAllObjects().length - generatedObjects.length;
	AbstractDungeonObject obj1;
	try {
	    obj1 = app.getDungeonManager().getDungeon().getCell(gridX - 1, gridY - 1,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
    ...
    private void autoGenerateTransitions2(final AbstractDungeonObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	...
	try {
	    obj2 = app.getDungeonManager().getDungeon().getCell(gridX, gridY - 1,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	} catch (final ArrayIndexOutOfBoundsException aioob2) {
	    obj2 = new EmptyVoid();
	}
    ...
    private void autoGenerateTransitions3(final AbstractDungeonObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	...
	try {
	    obj3 = app.getDungeonManager().getDungeon().getCell(gridX + 1, gridY - 1,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	} catch (final ArrayIndexOutOfBoundsException aioob2) {
	    obj3 = new EmptyVoid();
	}
    ...
    private void autoGenerateTransitions4(final AbstractDungeonObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	...
	try {
	    obj4 = app.getDungeonManager().getDungeon().getCell(gridX - 1, gridY,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	} catch (final ArrayIndexOutOfBoundsException aioob2) {
	    obj4 = new EmptyVoid();
	}
    ...
    private void autoGenerateTransitions6(final AbstractDungeonObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	...
	try {
	    obj6 = app.getDungeonManager().getDungeon().getCell(gridX + 1, gridY,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	} catch (final ArrayIndexOutOfBoundsException aioob2) {
	    obj6 = new EmptyVoid();
	}
    ...
    private void autoGenerateTransitions7(final AbstractDungeonObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	...
	try {
	    obj7 = app.getDungeonManager().getDungeon().getCell(gridX - 1, gridY + 1,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	} catch (final ArrayIndexOutOfBoundsException aioob2) {
	    obj7 = new EmptyVoid();
	}
    ...
    private void autoGenerateTransitions8(final AbstractDungeonObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	...
	try {
	    obj8 = app.getDungeonManager().getDungeon().getCell(gridX, gridY + 1,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	} catch (final ArrayIndexOutOfBoundsException aioob2) {
	    obj8 = new EmptyVoid();
	}
    ...
    private void autoGenerateTransitions9(final AbstractDungeonObject instance, final int gridX, final int gridY) {
	// Generate transitions automatically
	if (!instance.isOfType(TypeConstants.TYPE_GENERATION_ELIGIBLE)) {
	    return;
	}
	...
	try {
	    obj9 = app.getDungeonManager().getDungeon().getCell(gridX + 1, gridY + 1,
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	} catch (final ArrayIndexOutOfBoundsException aioob2) {
	    obj9 = new EmptyVoid();
	}
	...
	try {
		...
		if (mo2 == null) {
		    DungeonDiver4.getApplication().showMessage("This object has no properties");
		} else {
		    this.checkTwoWayTeleportPair(this.getLocationManager().getEditorLocationZ());
		    this.updateUndoHistory(this.savedDungeonObject, gridX, gridY,
			    ...
			    this.getLocationManager().getEditorLocationE());
		    app.getDungeonManager().getDungeon().setCell(mo2, gridX, gridY,
			    this.getLocationManager().getEditorLocationZ(),
			    this.getLocationManager().getEditorLocationE());
		    this.checkStairPair(this.getLocationManager().getEditorLocationZ());
		    this.checkMenus();
	...
	final Application app = DungeonDiver4.getApplication();
	final AbstractDungeonObject mo1 = app.getDungeonManager().getDungeon().getCell(
	...
	String name2, name3;
	try {
	    final AbstractDungeonObject mo2 = app.getDungeonManager().getDungeon().getCell(
	...
	try {
	    final AbstractDungeonObject mo3 = app.getDungeonManager().getDungeon().getCell(
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    z - 1, DungeonConstants.LAYER_OBJECT);
	    name3 = mo3.getName();
	} catch (final ArrayIndexOutOfBoundsException e) {
	...
	case STAIRS_UP:
		...
		app.getDungeonManager().getDungeon().setCell(new StairsDown(),
			this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
			this.getLocationManager().getEditorLocationZ() + 1, DungeonConstants.LAYER_OBJECT);
	    } catch (final ArrayIndexOutOfBoundsException e) {
		// Do nothing
	    }
	    break;
	...
	if (name1.equals("Two-Way Teleport")) {
	    ...
	    final AbstractDungeonObject mo2 = app.getDungeonManager().getDungeon().getCell(destX, destY, destZ,
		    DungeonConstants.LAYER_OBJECT);
	    name2 = mo2.getName();
	...
	final Application app = DungeonDiver4.getApplication();
	app.getDungeonManager().getDungeon().setCell(new Empty(), destX, destY, destZ, DungeonConstants.LAYER_OBJECT);
    }
	...
	final Application app = DungeonDiver4.getApplication();
	final String choice = CommonDialogs.showInputDialog("Edit What?", "Editor",
		DungeonEditorLogic.conditionalChoices, DungeonEditorLogic.conditionalChoices[0]);
	if (choice != null) {
	...
	case TELEPORT_TYPE_N_WAY:
	    ...
	    DungeonDiver4.getApplication().showMessage("Click to set teleport destination #" + (this.nWayDestID + 1));
	    break;
	...
	case TELEPORT_TYPE_INVISIBLE_BLOCK:
	    DungeonDiver4.getApplication().showMessage("Click to set teleport destination");
	    break;
	...
	DungeonDiver4.getApplication().showMessage("Click to set metal button target");
	final Application app = DungeonDiver4.getApplication();
	...
	DungeonDiver4.getApplication().showMessage("Pick treasure chest contents");
	...
	try {
	    tc = (TreasureChest) app.getDungeonManager().getDungeonObject(
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	...
	try {
	    app.getDungeonManager().getDungeon().getCell(destX, destY, destZ, DungeonConstants.LAYER_OBJECT);
	} catch (final ArrayIndexOutOfBoundsException ae) {
	...
	case TELEPORT_TYPE_N_WAY:
	    ...
	    break;
	case TELEPORT_TYPE_GENERIC:
	    app.getDungeonManager().getDungeon().setCell(new Teleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_INVISIBLE_GENERIC:
	    app.getDungeonManager().getDungeon().setCell(new InvisibleTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_ONESHOT:
	    app.getDungeonManager().getDungeon().setCell(new OneShotTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_INVISIBLE_ONESHOT:
	    app.getDungeonManager().getDungeon().setCell(new InvisibleOneShotTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_TWOWAY:
	    app.getDungeonManager().getDungeon().setCell(new TwoWayTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    this.pairTwoWayTeleport(destX, destY, destZ);
	    break;
	case TELEPORT_TYPE_CHAIN:
	    app.getDungeonManager().getDungeon().setCell(new ChainTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_INVISIBLE_CHAIN:
	    app.getDungeonManager().getDungeon().setCell(new InvisibleChainTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_ONESHOT_CHAIN:
	    app.getDungeonManager().getDungeon().setCell(new OneShotChainTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_INVISIBLE_ONESHOT_CHAIN:
	    app.getDungeonManager().getDungeon().setCell(new InvisibleOneShotChainTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_BLOCK:
	    app.getDungeonManager().getDungeon().setCell(new BlockTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_INVISIBLE_BLOCK:
	    app.getDungeonManager().getDungeon().setCell(new InvisibleBlockTeleport(destX, destY, destZ),
		    this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		    this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	    break;
	default:
	...
	app.getDungeonManager().getDungeon().setCell(new MetalButton(destX, destY, destZ, destW),
		this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	this.meg.swapFromMetalButtonHandler();
	this.checkMenus();
	...
	this.enableOutput();
	final Application app = DungeonDiver4.getApplication();
	final AbstractDungeonObject contents = this.containableObjects[this.meg.getTreasurePicked()];
	app.getDungeonManager().getDungeon().setCell(new TreasureChest(contents),
		this.getLocationManager().getEditorLocationX(), this.getLocationManager().getEditorLocationY(),
		this.getLocationManager().getCameFromZ(), DungeonConstants.LAYER_OBJECT);
	this.checkMenus();
	...
	try {
	    ...
	    app.getDungeonManager().getDungeon().setStartFloor(this.getLocationManager().getEditorLocationZ());
	    DungeonDiver4.getApplication().showMessage("Start point set.");
	} catch (final ArrayIndexOutOfBoundsException aioob) {
	    app.getDungeonManager().getDungeon().restoreStart();
	    DungeonDiver4.getApplication().showMessage("Aim within the dungeon");
	}
	// Swap event handlers
	...
	if (saved) {
	    ...
	    if (success) {
		this.fixLimits();
		app.getDungeonManager().getDungeon().fillLevel(PreferencesManager.getEditorDefaultFill(), new Empty());
	...
	if (this.confirmNonUndoable("overwrite the active level with default data")) {
	    DungeonDiver4.getApplication().getDungeonManager().getDungeon().fill();
	...
	if (this.confirmNonUndoable("overwrite the active level with random data")) {
	    if (DungeonDiver4.getApplication().getMenuManager().useFillRuleSets()) {
		DungeonDiver4.getApplication().getDungeonManager().getDungeon().fillLevelRandomlyCustom();
	    } else {
		DungeonDiver4.getApplication().getDungeonManager().getDungeon().fillLevelRandomly();
	    }
	    DungeonDiver4.getApplication().showMessage("Level randomly filled.");
	    DungeonDiver4.getApplication().getDungeonManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (this.confirmNonUndoable("overwrite the active floor within the active level with random data")) {
	    ...
	    DungeonDiver4.getApplication().showMessage("Floor randomly filled.");
	    DungeonDiver4.getApplication().getDungeonManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (this.confirmNonUndoable("overwrite the active layer on the active level with default data")) {
	    ...
	    DungeonDiver4.getApplication().showMessage("Level and layer filled.");
	    DungeonDiver4.getApplication().getDungeonManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (this.confirmNonUndoable(
	    ...
	    DungeonDiver4.getApplication().getDungeonManager().getDungeon().fillFloorAndLayer(
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	    DungeonDiver4.getApplication().showMessage("Floor and layer filled.");
	    DungeonDiver4.getApplication().getDungeonManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (this.confirmNonUndoable("overwrite the active layer on the active level with random data")) {
	    ...
	    DungeonDiver4.getApplication().showMessage("Level and layer randomly filled.");
	    DungeonDiver4.getApplication().getDungeonManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (this.confirmNonUndoable(
	    ...
	    if (DungeonDiver4.getApplication().getMenuManager().useFillRuleSets()) {
		DungeonDiver4.getApplication().getDungeonManager().getDungeon().fillFloorAndLayerRandomlyCustom(
			this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	    } else {
		DungeonDiver4.getApplication().getDungeonManager().getDungeon().fillFloorAndLayerRandomly(
			this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	    }
	    DungeonDiver4.getApplication().showMessage("Floor and layer randomly filled.");
	    DungeonDiver4.getApplication().getDungeonManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (input1 != null) {
		...
		if (input3 != null) {
			...
			success = app.getDungeonManager().getDungeon().addLevel(levelSizeX, levelSizeY, levelSizeZ);
			if (success) {
			    this.fixLimits();
			    ...
			    app.getDungeonManager().getDungeon().fillLevel(PreferencesManager.getEditorDefaultFill(),
				    new Empty());
			    // Save the entire level
			    app.getDungeonManager().getDungeon().save();
	...
	input1 = CommonDialogs.showTextInputDialogWithDefault("Number of rows (" + minR + "-" + maxR + ")?", msg,
		Integer.toString(app.getDungeonManager().getDungeon().getRows()));
	if (input1 != null) {
	    input2 = CommonDialogs.showTextInputDialogWithDefault("Number of columns (" + minC + "-" + maxC + ")?", msg,
		    Integer.toString(app.getDungeonManager().getDungeon().getColumns()));
	    if (input2 != null) {
		...
		if (input3 != null) {
			...
			app.getDungeonManager().getDungeon().resize(levelSizeX, levelSizeY, levelSizeZ);
			this.fixLimits();
			this.getViewManager()
	...
	input = CommonDialogs.showTextInputDialog(
		"Level Number (1-" + app.getDungeonManager().getDungeon().getLevels() + ")?", "Remove Level");
	if (input != null) {
	    try {
		level = Integer.parseInt(input);
		if (level < 1 || level > app.getDungeonManager().getDungeon().getLevels()) {
	...
	if (!this.goToDestMode) {
	    ...
	    DungeonDiver4.getApplication().showMessage("Click a teleport to go to its destination");
	}
    }
	...
	if (this.goToDestMode) {
	    ...
	    final AbstractDungeonObject there = DungeonDiver4.getApplication().getDungeonManager()
		    .getDungeonObject(locX, locY, locZ, DungeonConstants.LAYER_OBJECT);
		...
		DungeonDiver4.getApplication().showMessage("This object does not have a destination.");
	    }
	}
    }
	...
	if (x != -1 && y != -1 && z != -1 && w != -1) {
	    final AbstractDungeonObject oldObj = app.getDungeonManager().getDungeonObject(x, y, z, e);
